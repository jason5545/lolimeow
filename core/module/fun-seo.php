...
function boxmoe_google_submit($post_ID) {
    if (get_post_status($post_ID) == 'publish') {
        ...
        $sitemap_url = get_option('home') . '/sitemap.xml';
...
function boxmoe_yandex_submit($post_ID) {
    if (get_post_status($post_ID) == 'publish') {
        $YANDEX_API_KEY = get_boxmoe('boxmoe_yandex_api_key');
        $sitemap_url = get_option('home') . '/sitemap.xml';
        $api_url = 'https://webmaster.yandex.ru/ping?sitemap=' . urlencode($sitemap_url);
        $response = wp_remote_get($api_url);
        if(!is_wp_error($response) && $response['response']['code'] == 200){
            add_post_meta($post_ID, 'YandexSubmit', '推送成功', true);
        }
    }
}
add_action('publish_post', 'boxmoe_yandex_submit');
}
...
function boxmoe_description() {
    ...
    if ( is_singular() ) {
        if(get_boxmoe('boxmoe_post_keywords_description_switch') ) {
            $description_post = trim(get_post_meta($post->ID, 'description', true));
            if( $description_post ){
                $description = $description_post;
            }else{
                $description = trim(wp_title('',FALSE));
                if( get_the_subtitle(false) ){
                    $description .= get_the_subtitle(false);
                }
                $description = $description?$description:$blog_name;
            }
        }else{
            $description = trim(wp_title('',FALSE));
            if( get_the_subtitle(false) ){
        ...
        $description = $description?$description:'標籤頁 - '.$blog_name;
    } elseif ( is_category() ) { 
        $description = trim(strip_tags(category_description()));

        if( $description && strstr($description, '::::::') ){
            $desc = explode('::::::', $description);
            if( isset($desc[1]) && $desc[1] && !empty($desc[1]) ) {
                $description = trim($desc[1]);
            }
        }

        $description = $description?$description:'分類頁 - '.$blog_name;
    } elseif ( is_archive() )  { 
...
// 網站社交媒體描述輸出（無HTML標籤）--------------------------boxmoe.com--------------------------
function boxmoe_seo_description() {
    global $new_description;
    if( $new_description ) {
        return esc_attr($new_description);
    }
        ...
        if(get_boxmoe('boxmoe_post_keywords_description_switch') ) {
            ...
            if( $description_post ){
                ...
                if (has_excerpt()) {
                    ...
                    $description = mb_strimwidth(strip_tags(strip_shortcodes($post->post_content)), 0, 220, '...');
                }
            }
        }else{
            if (has_excerpt()) {
